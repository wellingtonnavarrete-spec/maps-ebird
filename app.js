...
function simularNavegacionEnVivo(coordenadasRuta) {
    ...
    const intervalo = setInterval(() => {
        if (index >= coordenadasRuta.length - 1) {
            clearInterval(intervalo);
            console.log("Simulación de ruta finalizada.");
            return;
        }

        const pActual = coordenadasRuta[index];
        const pSiguiente = coordenadasRuta[index + 1];

        // Calcular bearing (rumbo) hacia el siguiente punto
        const dLon = (pSiguiente[0] - pActual[0]) * Math.PI / 180;
        const lat1 = pActual[1] * Math.PI / 180;
        const lat2 = pSiguiente[1] * Math.PI / 180;
        const y = Math.sin(dLon) * Math.cos(lat2);
        const x = Math.cos(lat1) * Math.sin(lat2) - Math.sin(lat1) * Math.cos(lat2) * Math.cos(dLon);
        const rumbo = (Math.atan2(y, x) * 180 / Math.PI + 360) % 360;

        // Mover la cámara simulando avance de auto
        map.easeTo({
            center: pActual,
            zoom: 17,
            pitch: 65,
        ...
        index++;
    }, 1000); // Avanza un punto cada 1 segundo
}

// Para activarlo en consola tras trazar una ruta:
// simularNavegacionEnVivo(directions.getRoute()[0].geometry.coordinates);
